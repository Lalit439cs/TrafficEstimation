...
void playvideo(int startframe, int stopframe) {
	...
	while (cap.read(vidframe) == true) {
		...
		string WindowName = "Video" + std::to_string(startframe);
		imshow(WindowName, vidframe);
		waitKey(30);
		prevframe = vidframe;
	}
}
...
void startprocess() {
	...
	int stopframe1 = 1500 / 2;
	int stopframe2 = 1500;
	thread th1(playvideo,0,stopframe1);
	thread th2(playvideo, stopframe1, stopframe2);
	th1.join();
	th2.join();
	destroyAllWindows();
}
