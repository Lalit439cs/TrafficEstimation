...
void playvideo() {
	...
	framecount = 5;//remove later
	//cin >> framecount;
	int lastmcount = 0, lastqcount = 0;
	int movingcount=0;
	int queuecount=0;
	ifstream file("out.csv");
	file << "Frame Number" << "," << "Moving Count" << "," << "Queue Count" << "\n";
	while (cap.read(vidframe) == true) {
		...
		if ((prevframe.empty() == false)&& (framenum % framecount == 0)) {
			movingcount = 0;
			queuecount = 0;
		...
		file << framenum << "," << movingcount << "," << queuecount << "\n";
...
int main() {
	//cout << "Enter traffic image and video directory and name in separate lines. Also enter parameter:\n";
	//cin >> empname;
	//cin >> videoloc;
