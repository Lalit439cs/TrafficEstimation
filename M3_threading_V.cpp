...
void* subplayvideo( void* arg){
	...
	int start= ith * gap;
	Mat flow;
	if (ith==numthreads-1){last=vidframe.rows;}
	Mat curframe=vidframe(Range(start,last),Range::all() );
	int movingcountt = 0;
		int queuecountt = 0;
	if (prevframe.empty() == false) {
			Mat prevpart=prevframe( Range(start,last),Range::all());
...
int main() {
	...
	ofstream file("M3_threading_V"+to_string(numthreads)+".csv");
