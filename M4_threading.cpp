...
ofstream file("M4_threading"+to_string(numthreads)+".csv");
void* subplayvideo( void* arg){
	...
	int last= (ith+1) * gap;
	int start= ith * gap;
	if (ith==numthreads-1){last=totalframe;}
	VideoCapture capp(videoloc);
	capp.set(CAP_PROP_POS_FRAMES, start);
	//ofstream out("C:/Users/User/Desktop/TrafficCV/out.txt");
	//out << "Frame Number" << "," << "Moving Density" << "," << "Queue Density" << "\n";
	while (capp.read(vidframe)==true) {
		...
		int movingcount = 0;
		int queuecount = 0;
		int framenum=capp.get(CAP_PROP_POS_FRAMES);
		if (framenum == last) {
			if (ith !=numthreads-1){
			break;
			}
...
int main() {
	...
	totalframe = cap.get(CAP_PROP_FRAME_COUNT);
	gap= totalframe/numthreads;	
	globalcount=0;
